...
/*
 * map_fold_listas2 inmediatamente después de transformar el nodo de cada lista 
 * mediante la función visitante transformator le aplica el operador al nodo modificado
 * logrando así una mejor optimización en donde la cantidad de ciclos pasará de ser n
 * en vez de 2n, que sería el caso de aplicar fold_listas al final de la función en el 
 * retorno de la misma. Básicamente se aplican las dos funciones en el mismo bucle
 */
int map_fold_listas2(Snodo* list, Transformacion transformator, Operador operator);

/*
 * Funcion que libera toda la lista, incluyendo el campo datos, donde se libera la memoria
 * dinámica empleada en provincias, la memoria pedida para la estructura y la memoria del
 * nodo en sí
 */
